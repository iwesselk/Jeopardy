...
export class Jeopardy extends Component {
    ...
    constructor(props) {
        ...
        let queryID = qs.parse(this.props.location.search)["JeopardyID"];
        console.log("Querry ID is " + queryID);
        console.log("Address checked " + "./api/puzzles/"+(queryID||"first"));
        xhttp.open("get", "./api/puzzles/"+(queryID||"first"), false);
        xhttp.send();
        console.log("The response was " + xhttp.response);
        let futureboard = JSON.parse(xhttp.response)||[];
        this.state = {
            open: null,
            teams: [
                {
